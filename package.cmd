...
powershell.exe -NoProfile -Command "Compress-Archive -LiteralPath '%ROOT%dist\MV Media Downloader.exe','%ROOT%dist\MV Media Downloader Updater.exe','%ROOT%dist\yt-dlp.exe','%ROOT%dist\yt-dlp-plugins','%ROOT%README.md' -DestinationPath '%PACKAGE%' -CompressionLevel Optimal"
if errorlevel 1 exit /b 1
